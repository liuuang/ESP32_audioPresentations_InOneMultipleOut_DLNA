...
void loop() {
  ...
  if (pkt >= 8) {
    uint8_t buf[1472];
    int len = udp.read(buf, 1472);
    uint32_t seq; uint16_t off, sz;
    memcpy(&seq, buf, 4);
    memcpy(&off, buf+4, 2);
    memcpy(&sz, buf+6, 2);
    if (seq != currentSeq) { currentSeq = seq; framePos = 0; }
    if (off + sz <= FRAME_BYTES) {
      memcpy(frameBuf + off, buf + 8, sz);
      framePos += sz;
    }
    if (framePos >= FRAME_BYTES) {
      size_t w;
      i2s_write(I2S_NUM_0, frameBuf, FRAME_BYTES, &w, portMAX_DELAY);
      framePos = 0;
    }
  }
}